...
class RequirementsLifecycleTest(unittest.TestCase):
    ...
    def test_complete_student_and_admin_lifecycle(self):
        ...
        update_response = self.client.patch(f"/api/admin/properties/{property_id}", headers=moderator_headers, json={"bedrooms": 3, "rentMin": 975, "amenities": ["Parking", "Laundry"], "parkingCost": 75, "insuranceCost": 18, "averageUtilities": 110})
        self.assertEqual(update_response.status_code, 200)
        self.assertEqual(update_response.get_json()["listing"]["bedrooms"], 3)
        self.assertEqual(update_response.get_json()["listing"]["parkingCost"], 75)
